...
impl<
    ...
    pub fn load_file_bytes(&mut self, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        ...
        for i in 0..inode.blocks_used() { 
            let block_num = inode.blocks[i] as usize; 
            self.disk.read(block_num, &mut self.block_buffer).unwrap();
            let end = cursor + BLOCK_SIZE; 
            self.file_content_buffer[cursor..end].copy_from_slice(&self.block_buffer); 
            cursor+= BLOCK_SIZE; 
        }
    ...
    pub fn save_file_bytes(&mut self, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        ...
        for i in 0..inode.blocks_used() { 
            let block_num = inode.blocks[i] as usize; 
            self.block_buffer.copy_from_slice(&self.file_content_buffer[cursor..cursor + BLOCK_SIZE]); 
            self.disk.write(block_num, &self.block_buffer).unwrap(); 
            cursor += BLOCK_SIZE; 
        }
        // For each inode block in use
        //   Figure out the disk block referenced by that inode block
    ...
    pub fn activate_bit(&mut self, bit: usize, block: usize) {
        ...
        let byte = bit / 8; 
        let bit_index = bit % 8; 
        self.block_buffer[byte] |= 1 << bit_index; 
    ...
    fn request_data_block(&mut self) -> anyhow::Result<u8, FileSystemError> {
        ...
        for i in 0..self.num_data_blocks() {
            ...
            let byte = block / 8;
            let bit = block % 8;
            if self.block_buffer[byte] & (1 << bit) == 0 {
    ...
    pub fn inode_table_inode(&mut self) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        let mut inode = Inode { 
            bytes_stored: self.num_inode_entries() as u16, blocks: [0; MAX_FILE_BLOCKS], 
        };
        for i in 0..self.num_inode_blocks() { 
            inode.blocks[i] = (INODE_TABLE_START + i) as u8
        }
        inode
    ...
    // fn update_inode_table(&self, offset: usize, table_buffer: &mut [u8]) { 
       // table_buffer[offset] = self.inode.bytes_stored as u8; 
       // table_buffer[offset + 1] = (self.inode.bytes_stored >> 8) as u8; 
      //  for (i, &block) in self.inode.blocks.iter().enumerate() { 
  ...
  //  }
    ...
    fn mark_inode_blocks_in_use(&mut self) {
        for i in 0..self.num_inode_blocks() { 
            self.activate_bit(INODE_TABLE_START + i, DATA_FULL_BLOCK);
        }
    }
    ...
    fn initialize_new_file(
        ...
        let mut inode = Inode { 
            bytes_stored: 0, blocks: [0; MAX_FILE_BLOCKS], 
        };
    ...
    pub fn inode_for(
        ...
        for i in 0..MAX_FILES_STORED { 
            let offset = i * MAX_FILENAME_BYTES; 
            let entry = &self.file_content_buffer[offset..offset + MAX_FILENAME_BYTES]; 
            let null_terminated = entry.iter().position(|&b| b == 0).unwrap_or(MAX_FILENAME_BYTES); 
            if &entry[..null_terminated] == filename.as_bytes() { 
    ...
    pub fn open_create_new(&mut self, filename: &str) -> anyhow::Result<usize, FileSystemError> {
        ...
        fn create_directory_entry(
            ...
            let next_offset = offset + MAX_FILENAME_BYTES;
            if next_offset > directory_inode.bytes_stored as usize {
        ...
        pub fn close(&mut self, fd: usize) -> anyhow::Result<(), FileSystemError> {
            let Some(info) = &self.open[fd] else {
                return Err(FileSystemError::FileNotOpen);
            };
            let inode_num = info.inode_num;
            let writing = info.writing;
            let inode = info.inode.clone();
            if writing {
                self.save_inode(inode_num, &inode);
            }
            self.open[fd] = None;
        ...
        pub fn read(&mut self, fd: usize, buffer: &mut [u8]) -> anyhow::Result<usize, FileSystemError> {
            ...
            while count < buffer.len() && info.offset < info.inode.bytes_stored as usize {
                if info.offset % BLOCK_SIZE == 0 {
                    ...
                    self.disk.read(block, &mut info.block_buffer).unwrap();
                }
                buffer[count] = info.block_buffer[info.offset % BLOCK_SIZE];
                info.offset += 1;
                if info.offset % BLOCK_SIZE == 0 {
    ...
    pub fn write(&mut self, fd: usize, buffer: &[u8]) -> anyhow::Result<(), FileSystemError> {
        for &b in buffer {
            {
                ...
                info.block_buffer[info.offset % BLOCK_SIZE] = b;
                info.offset += 1;
            }
            let block_full;
            let current_block;
            ...
            if block_full {
                let (block_num, buffer_copy) = {
                    let info = self.open[fd].as_ref().unwrap();
                    (
                        info.inode.blocks[current_block] as usize,
                        info.block_buffer,
                    )
                };
                ...
                let needs_new_block;
                {
                    let info = self.open[fd].as_ref().unwrap();
                    needs_new_block =
                        info.current_block + 1 < MAX_FILE_BLOCKS
                            && info.inode.blocks[info.current_block + 1] == 0;
                }
        ...
        // Create an entry for the file in the file table.car
        // Return its file descriptor.
    }
...
impl<const MAX_BLOCKS: usize, const BLOCK_SIZE: usize> Inode<MAX_BLOCKS, BLOCK_SIZE> {
    ...
    pub fn update_inode_table(&self, offset: usize, table_buffer: &mut [u8]) {
        table_buffer[offset..offset+2].copy_from_slice(&self.bytes_stored.to_le_bytes());
        for i in 0..MAX_BLOCKS {
